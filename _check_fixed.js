...
var FOOD_GUIDE = {
  ...
  marathon: {
    ...
    timing:  ["赛前 3 天碳水加载（每kg体重 8-10g）","赛前早餐：碳水为主，赛前 2-3h 吃完","LSD 训练中每 45-60min 补充能量胶+水","赛后 30min 黄金窗口：碳水+蛋白=4:1（如 巧克力奶）","比赛日：不要尝试没吃过的食物！"]
  }
};
...
// ============ 营养计算 ============
function calcNutrition(weight, height, age, gender, goal) {
  ...
  if (goal === "muscle") { targetCal = tdee + 300; protein = weight * 2.0; carb = weight * 4; fat = weight * 1; }
  else if (goal === "strength") { targetCal = tdee + 200; protein = weight * 1.8; carb = weight * 4; fat = weight * 1; }
  else if (goal === "cut") { targetCal = tdee - 400; protein = weight * 1.8; carb = weight * 2; fat = weight * 0.8; }
  else if (goal === "marathon") { targetCal = tdee + 200; protein = weight * 1.4; carb = weight * 6; fat = weight * 1; }
  else if (goal === "cardio") { targetCal = tdee + 100; protein = weight * 1.4; carb = weight * 5; fat = weight * 1; }
  else { targetCal = tdee; protein = weight * 1.4; carb = weight * 3; fat = weight * 0.8; }
  return {
...
function doGenerateInternal(goal, level, days, equip, trainingDays, schedule, cfg, goalCfg) {
  ...
  var nutrition = calcNutrition(0, 0, 0, '男', goal); // 体重等参数非关键，用默认值
  try {
    ...
    nutrition = calcNutrition(weight, height, age, gender || '男', goal);
  } catch(e) {}
  var html = "";
  ...
  var dayCalBurns = trainingDays.map(function(day){
    if (!day.exes) return 0;
    return day.exes.reduce(function(sum, ex){ if (ex._injured) return sum; var diff = ex.diff || '中级'; var m = ex.m || '胸'; return sum + (estimateCalories ? estimateCalories(diff, m) : 100); }, 0);
  });
...
function buildMarathonPlan(level, days, equip, wOff) {
  ...
  var lsdInc  = {beginner:3, intermediate:2.5, advanced:2}[level] || 3;
  ...
  function mkRun(name, distance, pace, effort, desc, tips) {
    return { n: name + " " + distance + runEnv, m: "跑步", eq: equip, diff: effort, desc: desc, tips: tips, pace: pace, isMarathon: true };
  }
  ...
  if (days === 4) {
    if (phase === "base") {
      ...
      plan = [
        ...
        {name:"短间歇 (激活)", exes:[mkRun("间歇", intervalSpec, cfg.intervalPace+"/km", "中级", "神经激活，保持速度感，强度降低", "组数比平时少，重点保持腿部轻快感")]},
    ...
    if (phase === "base") {
      ...
      plan = [
        ...
        {name:"短间歇 (激活)", exes:[mkRun("间歇", intervalSpec, cfg.intervalPace+"/km", "中级", "保持速度感", "组数减少，强度保持")]},
    ...
    if (phase === "base") {
      ...
      plan = [
        ...
        {name:"短间歇 (激活)", exes:[mkRun("间歇", intervalSpec, cfg.intervalPace+"/km", "中级", "激活神经系统，保持速度感", "组数减少，强度保持，感受轻快")]},
...
function renderSummary(goalName, levelName, equipName, days, goalCfg, sets, intensity, goal) {
  var presTag;
  if (goal === "marathon") {
    presTag = "周跑量 " + goalCfg.weeklyKms + "km";
...
function renderMarathonProgress(wkInfo, goalCfg, level, currentCfg) {
  ...
  var html = '<div class="week-info" style="background:linear-gradient(135deg,#fef3c7,#fde68a);border-left:4px solid '+currentPhase.color+';">'+
    ...
    ' · <strong>'+phaseDesc+'</strong> · 周跑量 '+goalCfg.weeklyKms+'km</div></div>';
...
function renderNutrition(n, goal, avgTrainBurn, maxTrainBurn, schedule, trainingDays, dayCalBurns) {
  ...
  if (nutriDays.length > 0) {
    var trainCal = restCal + avgTrainBurn;
    html += '<div class="nutri-dynamic" style="margin-top:10px;padding:10px 12px;background:var(--bg);border-radius:10px;">'+
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">'+
        '<span style="font-size:12px;font-weight:600;color:var(--text);">🔥 训练日动态热量</span>'+
        '<span style="font-size:11px;color:var(--text3);">点击日程切换</span></div>';

    // 日程 chip 条
    html += '<div class="nutri-chips" style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px;">';
    // 默认：平均
    html += '<span class="nutri-chip active" data-ni="-1" onclick="selectNutriDay(\''+nutriPanelId+'\',-1,'+restCal+')" style="font-size:11px;padding:5px 8px;border-radius:14px;cursor:pointer;background:var(--primary);color:#fff;font-weight:600;">📊 平均</span>';
    nutriDays.forEach(function(nd, i) {
      html += '<span class="nutri-chip" data-ni="'+i+'" data-burn="'+nd.burn+'" data-name="'+nd.label+' '+nd.name+'" onclick="selectNutriDay(\''+nutriPanelId+'\','+i+','+restCal+')" style="font-size:11px;padding:5px 8px;border-radius:14px;cursor:pointer;border:1px solid var(--border);color:var(--text2);">'+
    ...
    html += '<div style="display:flex;gap:8px;">'+
      ...
      '<div style="flex:1;text-align:center;padding:8px 4px;background:var(--card);border-radius:8px;">'+
        '<div style="font-size:22px;font-weight:800;color:var(--text2);">'+restCal+'</div>'+
        '<div style="font-size:10px;color:var(--text3);">休息日 kcal</div></div></div>'+
      (maxTrainBurn > avgTrainBurn ? '<div style="font-size:10px;color:var(--text3);margin-top:6px;">💡 点击上方日程查看对应日热量，LSD日额外多补碳水</div>' : '')+
      '</div>';
  }
  ...
  // 示例食谱
  html += '</div><div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--border);">'+
    '<div style="font-size:13px;font-weight:700;color:var(--text);margin-bottom:8px;">🍽️ 今日示例食谱（'+n.protein+'g蛋白 / '+n.carb+'g碳水）</div>'+
    '<div style="font-size:12px;color:var(--text2);line-height:1.8;">'+
    '<b>早餐</b>：燕麦50g + 脱脂奶250ml + 水煮蛋2个 + 香蕉1根<br>'+
    '<b>加餐1</b>：希腊酸奶150g + 蓝莓一小把<br>'+
    '<b>午餐</b>：糙米饭150g + 鸡胸肉150g + 西兰花200g + 橄榄油5g<br>'+
    '<b>加餐2（训前）</b>：全麦面包1片 + 花生酱10g<br>'+
    '<b>晚餐（训后）</b>：红薯150g + 三文鱼/瘦牛肉120g + 菠菜200g<br>'+
    '<b>睡前</b>：牛奶250ml 或 酪蛋白粉1勺</div>'+
    '<div style="font-size:11px;color:var(--text3);margin-top:6px;">* 按目标微调：增肌多加1餐加餐，减脂去掉1餐加餐和睡前奶</div>';
...
// 营养面板日程切换
function selectNutriDay(panelId, idx, restCal) {
  ...
  if (!trainEl || !labelEl) return;

  var totalBurn = 0, label = '平均训练日 kcal';
  if (idx < 0) {
    // 平均：统计所有日程的 data-burn 取均值
    var count = 0;
    chips.forEach(function(c){
      var burn = parseInt(c.getAttribute('data-burn')) || 0;
      if (burn) { totalBurn += burn; count++; }
    });
    totalBurn = Math.round(totalBurn / (count || 1));
  } else {
    // 指定日程
    var selChip = panel.querySelector('.nutri-chip[data-ni="'+idx+'"]');
    totalBurn = parseInt(selChip ? selChip.getAttribute('data-burn') : 0) || 0;
    label = (selChip ? selChip.getAttribute('data-name') : '') + ' kcal';
  }
  trainEl.textContent = restCal + totalBurn;
...
function renderDayCard(dayLabel, day, sets, goalCfg, warmup, wkInfo, goal, dayCalBurn, levelCfg, dayIdx) {
  ...
  if (day.exes) {
    day.exes.forEach(function(ex, idx){
      ...
      var done = localStorage.getItem("fitbuddy_done_" + checkId) === "1";
      ...
      if (isMarathon && ex.isMarathon) {
        var distMatch = ex.n.match(/(\d+)km/);
        if (distMatch) distData = ' data-dist="'+distMatch[1]+'"';
        exPrescriptionHtml =
          '<div class="ex-sets">🏃 配速 ' + (ex.pace || '—') + '</div>'+
          '<div style="font-size:11px;color:var(--primary);margin-top:2px;font-weight:500;">' + (ex.tips || '') + '</div>';
      } else if (goal === 'cardio' && ex.m === '有氧') {
        var cdMatch = ex.n.match(/(\d+)km/);
        if (cdMatch) distData = ' data-dist="'+cdMatch[1]+'"';
...
function toggleDone(id) {
  ...
  if (isDone) {
    ...
    localStorage.removeItem("fitbuddy_done_" + id);
    ...
    if (found) {
      ...
      // 减去热量
      if (exDiff) {
        var cal = estimateCalories(exDiff, exM);
    ...
    localStorage.setItem("fitbuddy_done_" + id, "1");
    // 记录训练历史（动作信息已在上方获取）
...
function showEx(name) {
  ...
  var searchUrl = "https://search.bilibili.com/all?keyword=" + encodeURIComponent(ex.n + " 教学示范");
  var videoHtml = '<div class="modal-section-title">📹 教学视频</div>'+
    '<a class="video-link" href="'+searchUrl+'" target="_blank">📺 在 B站 搜索「'+ex.n+'」教学视频</a>';
...
// ============ 进度页 ============
// 估算单动作消耗热量（大卡，基于 70kg 标准体重）
function estimateCalories(diff, m) {
  // MET 值估算（根据难度和动作类型）
  var met = 4; // 默认中等强度
  if (diff === '初级') met = 3;
  else if (diff === '中级') met = 5;
  else if (diff === '高级') met = 7;
  // 有氧和腿部动作消耗更高
  if (m === '有氧') met += 2;
  if (m === '腿') met += 1;
  // 假设每个动作约 8 分钟（组间休息 + 训练）
  var minutes = 8;
  // 卡路里 = MET × 3.5 × 体重(kg) × 分钟 / 200
  // 以 70kg 为标准，实际可根据用户体重调整
  var weight = 70;
  // 尝试从用户资料读取体重
  try {
  ...
  var cal = Math.round(met * 3.5 * weight * minutes / 200);
  return cal;
}
...
function recordHistory(dist, exName, exDiff, exM) {
  ...
  // 计算本次动作热量
  var thisCal = (exName && exDiff) ? estimateCalories(exDiff, exM) : 0;
...
function renderProgress() {
  ...
  if (isRunning) {
    ...
    html += '<div class="progress-card">'+
      ...
      '<div class="progress-stat"><span class="progress-stat-label">目标训练日</span><span class="progress-stat-value">'+daysThisWeek+' 天/周</span></div>'+
      '<div style="margin-top:10px;"><div style="font-size:12px;color:var(--text3);margin-bottom:4px;">频次完成度</div>'+
      '<div class="progress-bar"><div class="progress-bar-fill" style="width:'+pct+'%;"></div></div>'+
      '<div style="font-size:12px;color:var(--text3);margin-top:4px;">'+pct+'%</div></div></div>';
  } else {
...
// 如果有上次生成的计划，直接渲染 HTML（不调用 doGenerate，避免按钮不存在的问题）
if (lastPlan && lastPlan.goal && lastPlan.trainingDays) {
  try {
    var _goal  = lastPlan.goal;
    var _level = lastPlan.level;
    var _days  = lastPlan.days;
    var _equip = lastPlan.equip;
    // 恢复表单 UI
    var _gEl = document.querySelector('input[name="goal"][value="'+_goal+'"]');
    if (_gEl) _gEl.checked = true;
    var _lEl = document.querySelector('input[name="level"][value="'+_level+'"]');
    if (_lEl) _lEl.checked = true;
    var _dEl = document.querySelector('input[name="days"][value="'+_days+'"]');
    if (_dEl) _dEl.checked = true;
    var _eEl = document.querySelector('input[name="equip"][value="'+_equip+'"]');
    if (_eEl) _eEl.checked = true;
    // 渲染计划 HTML
    _skipRebuild = true;
    doGenerate();
...
window.addEventListener('load', function() {
  ...
  if (planResult && !planResult.innerHTML.trim()) {
    planResult.innerHTML = '<div class="loading-overlay"><div style="font-size:48px;margin-bottom:12px;">🏋️</div><div class="loading-text">选择目标、水平和天数，点击「生成我的计划」开始</div></div>';
  }
});
